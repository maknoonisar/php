...
<!-----head------->
<?php include "headers/baggage_head.php" ?>
<!--end head------>
...
<!------------------------------------------------->
		...
		<!--output of model Crime scene detection-->
			<div class="crimeveddis" id="outputved" style="display:block; margin-top: -878px;">
			<?php
				// (B) GET LIST OF VIDEO FILES FROM GALLERY FOLDER
				$dir = __DIR__ . DIRECTORY_SEPARATOR . "output" . DIRECTORY_SEPARATOR;
				$images = glob($dir . "*.{mp4,avi,webm}", GLOB_BRACE);
 
				// (C) OUTPUT VIDEO 
				foreach ($images as $i) {
				printf(" <div style='text-align:right;margin-right:87px;margin-left:10px;'> <video  width='1500' height='870' src='output/%s'  controls autoplay muted/>  </div>", basename($i));
				}
			?>	
			</div>
		</div>
...
<script type="text/javascript">
    document.getElementById("savechngved").onclick = function () {
        location.href = "baggage_detect.php";
    };
</script>
