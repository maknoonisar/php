...
<!-----head------->
<?php include "headers/uav_summarization_head.php" ?>
<!--end head------>
...
<!------------------------------------------------->
		...
		<!--output of model Crime scene detection-->
			...
			<?php
				...
				$dir = __DIR__ . DIRECTORY_SEPARATOR . "uav_output_video_summarization" . DIRECTORY_SEPARATOR;
				$images = glob($dir . "*.{mp4,avi,webm}", GLOB_BRACE);
 
				// (C) OUTPUT VIDEO 
				foreach ($images as $i) {
				printf(" <center> <video width='1500' height='870' src='uav_output_video_summarization/%s'  controls autoplay muted/>  </center>", basename($i));
				}
			?>	
			</div>
		</div>
 ...
 function fetch_month1(hh)
            {
	            ...
	            $.ajax({
				type: 'POST',
				url: 'uav_product_video_summarization.php',
				data: { name: hh },
