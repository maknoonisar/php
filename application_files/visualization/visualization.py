                ...
                if obj_capture.shape[0] * obj_capture.shape[1] > 0 and obj_capture.shape[0] > 1 and obj_capture.shape[
                    ...
                    if process0 == 'Video+Licence plate':
                        ...
                        if process1 == 'Video+Licence plate+Text':
                            ...
                            lic_text_results = pd.DataFrame(columns=['File_name', 'Date', 'Time', 'Licence_plate_no', 'Flagged','count'])
                            if flag_plate is not None and len(lic_text_list) > 0:
                                ...
                                if ratio > 0.5:
                                    lic_results = lic_text_results.append({'File_name': lic_plate_filename, 'Date': str(self.date), 'Time': str(self.cur_time), 'Licence_plate_no': lic_text_list, 'Flagged': 'Yes','count':count}, ignore_index=True)
