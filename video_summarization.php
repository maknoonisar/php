...
<!-----head------->
<?php include "headers/video_summarization_head.php" ?>
<!--end head------>
...
<?php 
$local_dir = 'input_video_summarization/';
...
<!------------------------------------------------->
		...
		<!--output of model Crime scene detection-->
			...
			<?php
				...
				$dir = __DIR__ . DIRECTORY_SEPARATOR . "out_video_summarizer" . DIRECTORY_SEPARATOR;
				$images = glob($dir . "*.{mp4,avi,webm}", GLOB_BRACE);
 
				// (C) OUTPUT VIDEO 
				foreach ($images as $i) {
				printf(" <center> <video width='1500' height='870' src='out_video_summarizer/%s'  controls autoplay muted/>  </center>", basename($i));
				}
			?>	
			</div>
		</div>
 ...
 function fetch_month1(hh)
            {
	            ...
	            $.ajax({
				type: 'POST',
				url: 'file_product_video_summarization.php',
				data: { name: hh },
